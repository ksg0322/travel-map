# 🗺️ Travel Map (AI 기반 여행 경로 추천 서비스)

Google API를 활용하여 장소를 검색하고 사용자 맞춤형 여행 경로를 계획하여 시각화해주는 웹 애플리케이션입니다.

## 🚀 프로젝트 개요
사용자가 원하는 장소를 검색하고 저장하면, AI 여행 에이전트가 최적의 여행 경로를 계획하여 지도에 표시해줍니다. 
단순한 장소 추천을 넘어, 실제 이동 동선을 고려한 여행 계획을 경험할 수 있습니다.
...
- **거리 및 평점 필터**: 사이드 바에서 검색 반경과 최소 평점을 조절하여여 검색 결과를 필터링 할 수 있습니다.
- **카테고리 필터**: 검색바 우측의 호텔, 식당, 관광지 버튼으로 카테고리별로 검색 결과를 필터링 할 수 있습니다.
...
- **장소 추천 (Search Agent)**: 장소 추천 요청 시 검색 결과에서 거리나 리뷰 등을 종합하여 추천합니다.
